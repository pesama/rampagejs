/**
 * @class {Rampage.gui.items.RRadioOption}
 * @extends {Rampage.gui.items.RInput}
 * Represents a native HTML5 radio button
 * 
 * @author Pelayo Sanchez Margareto
 * @version 2.0.1-ALPHA
 * 
 * @param name {String} The name of the input (for grouping)
 * @param id {String} The ID of the input
 */
Rampage.gui.items.RRadioOption = function(name, id) {
	this.className = 'Rampage.gui.items.RRadioOption';
	this.extends(Rampage.gui.items.RInput);
	this.bind('radiooption');

	this.name(name);
	if(id) this.id(id);

	/**
	 * @type Boolean Whether the input is checked
	 */
	var checked = false;

	/**
	 * @type Rampage.gui.items.RButtonSet The group where the input belongs
	 */
	var group = null;
	...
	/**
	 * Sets and gets the group for the item
	 * @param grp {Rampage.gui.items.RButtonSet} The new group
	 * @return {Rampage.gui.items.RButtonSet} The group
	 */
